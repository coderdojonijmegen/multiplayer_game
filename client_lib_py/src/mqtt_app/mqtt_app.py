...
class MqttApp:
    ...
    def __init__(self, config: MqttConfig, client_id: str):
        ...
        self.client.will_set(f"clients/drone-game/{client_id}", "disconnected", retain=True)
    ...
    def _on_connect(self, client: Client, userdata, flags, rc) -> None:
        ...
        client.publish(f"clients/drone-game/{self.client_id}", f"connected since {datetime.now()}", retain=True)
